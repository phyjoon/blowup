...
\end{align} 
with the definitions \eqref{eq:param-np-sp} understood. For the case of 5d pure $\mathcal{N}=1$ SYM, with a possible non-zero Chern-Simons level $\kappa$, the correlation function turns out to be \cite{Nakajima:2005fg,Gottsche:2006tn} 
\begin{align}
...
\begin{align}
  \label{eq:cor5d-dmax}
  d_\text{max} = \begin{dcases}
  \textstyle h^\vee - \frac{1}{2}\sum_i I_2(\mathbf{R}_i) & \quad \text{if}\quad  G \neq SU(N)\\ \textstyle h^\vee - \frac{1}{2}\sum_i I_2(\mathbf{R}_i) & \quad \text{if}\quad  G = SU(N) \textcolor{red}{change}
\end{dcases}
\end{align}
...
\begin{align}
  ...
  (p_1p_2)^{(d+\frac{b}{2}) (\frac{\vec{k}\cdot\vec{k}}{2}) +\frac{\k}{6}d_{ijk}\,k^ik^jk^k} \nn\\&\times  e^{-(d+\frac{b}{2})(\vec{a}\cdot\vec{k})}\,
...
\begin{align}
  ...
  \Bigg( (p_1p_2)^{(d+\frac{h^\vee}{2} - \frac{\kappa_\text{eff}}{2}) (\frac{\vec{k}\cdot\vec{k}}{2}) +\frac{\k_\text{eff}}{6}d_{ijk}\,k^ik^jk^k} e^{-(d-\frac{\k_\text{eff}}{2})(\vec{a}\cdot\vec{k})}\,
...
\begin{align}
  \label{eq:1inst}
  Z_1 = p_1^{(d+\frac{b}{2})}Z^{(N)}_{1}  + p_2^{(d+\frac{b}{2})}  Z^{(S)}_1  + I_1^{(d)} \ , 
 \end{align}
 ...
 \begin{align}
  I_1^{(d)} = &\sum_{\vec{k} \in \Delta_l}  \Bigg( 
     (p_1p_2)^{(d+\frac{b}{2} )  +\frac{\k_\text{eff}}{6}d_{ijk}\,k^ik^jk^k} e^{-(d-\frac{\k_\text{eff}}{2})(\vec{a}\cdot\vec{k})}\,
    e^{-\frac{\k_\text{eff}}{2}d_{ijk}\,a^ik^jk^k}  \\& 
...
Prior to solving the equations  for $Z_1$, we recall that the formula \eqref{eq:recursion} is valid only in a certain range of $d$, for which $\langle \mathcal{O}_{\IP^1}^{d}\rangle = 1$. We want to narrow down the validity range of $d$ by doing a simple sanity check on \eqref{eq:1inst}. Specifically, we examine the double expansion of \eqref{eq:1inst} with respect to $y_{\text{phy},l} \ll \sqrt{p_1p_2} \ll 1$, identifying an exponent in $\sqrt{p_1p_2}$ of the leading term. 
The expansion of the single terms in \eqref{eq:1inst} can be written as
{\allowdisplaybreaks
\begin{align}
  I_1^{(d)} &\sim \prod_{l} y_{\text{phy},l}^{-I_2(\boldsymbol{R}_l)/2}  \left(  (p_1p_2)^{d+\frac{b}{2} + 1} g_0(\vec{a}) + \cdots \right) + \cdots  \nn \\
  Z_1 &\sim \prod_{l} y_{\text{phy},l}^{-I_2(\boldsymbol{R}_l)/2}  \left(  (p_1p_2)^{\frac{n}{2}} g_1(\vec{a}) + \cdots \right) + \cdots  \nn \\
  p_1^{(d+\frac{b}{2})}Z_1^{(N)} &\sim \prod_{l} y_{\text{phy},l}^{-I_2(\boldsymbol{R}_l)/2}  \left(  (p_1p_2)^{\frac{d}{2}+\frac{b}{4}+\frac{n}{4}} g_2(\vec{a}) + \cdots \right) + \cdots  \nn \\
  p_2^{(d+\frac{b}{2})}  Z^{(S)}_1&\sim \prod_{l} y_{\text{phy},l}^{-I_2(\boldsymbol{R}_l)/2}  \left(  (p_1p_2)^{\frac{d}{2}+\frac{b}{4}+\frac{n}{4}} g_3(\vec{a}) + \cdots \right) + \cdots.
\end{align}
where  
\begin{align}
  \label{eq:upperbound}
  n  = \begin{dcases}
    \textstyle h^\vee - \frac{1}{2} \sum_{l}I_2(\mathbf{R}_l)& \text{ for } G \neq SU(N)\\
    \textstyle h^\vee  - \frac{1}{2} \sum_{l}I_2(\mathbf{R}_l) - \k_\text{eff} & \text{ for } G = SU(N).
  \end{dcases}
\end{align}
except for the special case of $SU(N)_{|\kappa|<N}$ gauge theory without a hypermultiplet, whose $n$ enhances to $n = h^\vee + |\k|$ \cite{Gottsche:2006bm}. We note that for the equation \eqref{eq:1inst} to hold, some terms on the right-hand side should have the leading exponent with respect to $\sqrt{p_1 p_2}$ less than or equal to that of $Z_1$. Thus the following condition is  naturally imposed: 
\begin{align}
  \frac{n}{2} \geq  d + \frac{b}{2}.
\end{align}
which sets an upper bound on $d$. Similarly, a lower bound on $d$ can be found from another expansion of \eqref{eq:1inst}, with respect to $y_{\text{phy},l} \ll 1/\sqrt{p_1p_2} \ll 1$.\footnote{This is equivalent to assuming a different parameter regime $ 0 < a_i < -\e_+ < m$ for all $1\leq i \leq r$. In general, an explicit form of the 1-loop partition function \eqref{eq:1-loop-vec}--\eqref{eq:1-loop-hyp} can change depending on a parameter regime, thus affecting \eqref{eq:recursion}. However, all the above expressions remain valid under flipping a sign of $\epsilon_+$, such that we can simply study the double expansion of the single terms in \eqref{eq:1inst} with respect to $y_{\text{phy},l} \ll 1/\sqrt{p_1p_2} \ll 1$.} 
Each term in \eqref{eq:1inst} becomes
{\allowdisplaybreaks
\begin{align}
  I_1^{(d)} &\sim \prod_{l} y_{\text{phy},l}^{-I_2(\boldsymbol{R}_l)/2}  \left(  (1/p_1p_2)^{-d-\frac{b}{2} + 1} h_0(\vec{a}) + \cdots \right) + \cdots  \nn \\
  &+\prod_{l} y_{\text{phy},l}^{+I_2(\boldsymbol{R}_l)/2}  \left(  (p_1p_2)^{-d-\frac{b}{2} + 1}  h_1(\vec{a}) + \cdots \right) \nn \\
  Z_1 &\sim \prod_{l} y_{\text{phy},l}^{-I_2(\boldsymbol{R}_l)/2}  \left(  (p_1p_2)^{\frac{n'}{2} + \frac{1}{2}\sum_l I_2(\mathbf{R}_l)} h_2(\vec{a}) + \cdots \right) + \cdots  \nn \\
  &+\prod_{l} y_{\text{phy},l}^{+I_2(\boldsymbol{R}_l)/2}  \left(  (p_1p_2)^{\frac{n'}{2}} h_3(\vec{a}) + \cdots \right) \nn \\
  p_1^{(d+\frac{b}{2})}Z_1^{(N)} &\sim \prod_{l} y_{\text{phy},l}^{-I_2(\boldsymbol{R}_l)/2}  \left(  (p_1p_2)^{-\frac{d}{2}-\frac{b}{4}+\frac{n'}{4}+ \frac{1}{4}\sum_l I_2(\mathbf{R}_l)} h_4(\vec{a}) + \cdots \right) + \cdots  \nn \\
  &+\prod_{l} y_{\text{phy},l}^{+I_2(\boldsymbol{R}_l)/2}  \left(  (p_1p_2)^{-\frac{d}{2}-\frac{b}{4}+\frac{n'}{4}} h_5(\vec{a}) + \cdots \right) \nn \\
  p_2^{(d+\frac{b}{2})}  Z^{(S)}_1&\sim \prod_{l} y_{\text{phy},l}^{-I_2(\boldsymbol{R}_l)/2}  \left(  (p_1p_2)^{-\frac{d}{2}-\frac{b}{4}+\frac{n'}{4}+ \frac{1}{4}\sum_l I_2(\mathbf{R}_l)} h_6(\vec{a}) + \cdots \right) + \cdots \nn\\
  &+\prod_{l} y_{\text{phy},l}^{+I_2(\boldsymbol{R}_l)/2}  \left(  (p_1p_2)^{-\frac{d}{2}-\frac{b}{4}+\frac{n'}{4}} h_5(\vec{a}) + \cdots \right) 
\end{align}
where  
\begin{align}
  ...
  \end{dcases}
\end{align}
So actually the stronger bound on $d$ 


\pagebreak


% Let us narrow down the validity regime of $d$ by doing a sanity check on the expected formula \eqref{eq:recursion} at one instanton:
% To do this, let us separate $n$-instanton partition function $Z_n$ from the above expression. 
\begin{align}
...
\begin{align}
  ...
  % &\,\times e^{(\frac{h^\vee}{2}-\sum_{l=1}^{N_f}\frac{I_2(\boldsymbol{R}_l)}{4})(\vec{a}\cdot\vec{k})+\sum_{l=1}^{N_f}\frac{I_3(\boldsymbol{R}_l)}{4}d_{ijk}\,a^ik^jk^k}\prod_{l=1}^{N_f}\left(M_l^{-\frac{I_2(\boldsymbol{R}_l)}{4}(\vec{k}\cdot\vec{k})}\right)(p_1p_2)^{\sum_{l=1}^{N_f}\frac{I_2(\boldsymbol{R}_l)}{8}(\vec{k}\cdot\vec{k})-\frac{I_3(\boldsymbol{R}_l)}{12}d_{ijk}\,k^ik^jk^k}\nn\\
  % &\times\frac{\prod_{l=1}^{N_f}\prod_{\vec{\omega}\in\boldsymbol{R}_l}\mathcal{L}(e^{-m_l^\text{phy}-\vec{a}\cdot\vec{\omega}},\vec{k}\cdot\vec{\omega})}{\prod_{\vec{\alpha}\in\Delta}\mathcal{L}(e^{-\vec{a}\cdot\vec{\alpha}},\vec{k}\cdot\vec{\alpha})}\nn\\
  % &\times Z_{\text{inst}}(\e_1,\e_2-\e_1,\vec{a}+\vec{k}\e_1,m-\frac{\e_1}{2},\Lambda+(d+\frac{b_0}{2})\e_1)\nn\\
  % &\times Z_{\text{inst}}(\e_1-\e_2,\e_2,\vec{a}+\vec{k}\e_2,m-\frac{\e_2}{2},\Lambda+(d+\frac{b_0}{2})\e_2)
\end{align}
...
% such as the ADHM quantum mechanics, topological vertex, or the modular bootstrap of partition function applied to their 6d origin, 
