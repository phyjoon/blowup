...
\end{align}
However, \eqref{eq:blowup-generic} is not enough to determine the partition function since there are three unknown functions and only one relation. To achieve this we need to consider a `correlation function' $\langle \mu(C) \ldots \mu(C)  \rangle$ of operators $\mu$ associated to the two-cycle $C$ in the blow-up $\hat{\mathbb{C}}^2$. In terms of the component fields of the Donaldson-twisted theory, it can be written as 
\begin{align} \label{eq:muC}
\mu(C) = \int d^4 x \left( \omega \wedge\, \phi F + \half \omega \wedge\psi \wedge \psi + H F \wedge F \right)  \ , 
\end{align}
where $\omega$ is the two-form dual to $C$ and $H = \e_1 |z_1|^2 + \e_2 |z_2|^2$ is the moment map for the $U(1)^2$ action. \textcolor{red}{(Q. Is $H$ correct?)}
...
\end{split}
\end{align}
and $\mu(\vec{k})$ is the contribution coming from the operator insertion $\mu(C)$. It is a function of $\vec{k}$ and also depends on $\vec{a}, \e_1,\e_2$ that we often abbreviate. Explicitly, it is written as
\begin{align}
 \exp \left( \mu(\vec{k}) \right) = \exp \left( \vec{k} \cdot \vec{a} +  \half \vec{k}\cdot\vec{k}\, (\e_1 + \e_2) \right) \ . 
\end{align}
\textcolor{red}{(add more explanation)}
\\
Notice that the effective instanton fugacity $q$ has to be shifted by equivariant parameters at both poles as $q \to q\, e^{t \e_{1, 2}}$. This is due to the term $H F\wedge F$ in \eqref{eq:muC} which makes the effective instanton parameter as $q e^{t(\e_1+\e_2)}$ At the north pole, $\e_1 + \e_2 \to \e_1 + (\e_2 - \e_1) = \e_2$. We need to compensate this part by shifting $q$ by $e^{t\e_1}$. Similarly, we need to shift the parameters at the south pole. 
...
It turns out piece of the \eqref{eq:1loopvec}, \eqref{eq:1loophyp} that are not inside the PE eventually cancel out due to $\vec{k} \to -\vec{k}$ and $\vec{a} \to -\vec{a}$ symmetry except for one single term for the hyper \textcolor{red}{(change)}. Introducing the symbols $n_1(\vec{k},\vec{w}) \equiv \sum_{\ell=1}^{\vec{k}\cdot \vec{w}} \ell$ and  $n_2(\vec{k},\vec{w}) \equiv \sum_{\ell=1}^{\vec{k}\cdot \vec{w}} (\ell^2-\ell)$ for brevity, we find
\begin{align}
 ...
 g(\vec{k})^{H} &= \prod_{\vec{w} \in R} \exp{\left((\vec{a}\cdot \vec{w}+{m^{\rm phy}) n_1(\vec{k},\vec{w})} + \frac{\e_1+\e_2}{2} n_2(\vec{k},\vec{w})\right)}\nn\\&
 \quad\quad\times 
 \textrm{PE} \left[ \frac{- e^{\vec{a} \cdot \vec{w}+ m^{\textrm{phy}}}}{(1-e^{\e_1})(1-e^{\e_2})} + \frac{e^{(\vec{a}+\vec{k}\e_1) \cdot \vec{w} + m^{\textrm{phy}}}}{(1-e^{\e_1})(1-e^{\e_2-\e_1})} + \frac{e^{(\vec{a}+\vec{k}\e_2) \cdot \vec{w} + m^{\textrm{phy}}}}{(1-e^{\e_1- \e_2})(1-e^{\e_2})} \right]
\end{align}
for the vector and hypermultiplets respectively. 
