...
, and especially the general 1-instaon expression 
$Z_1$ indeed produces 


We also obtain various
...
\cite{Keller:2012da, Huang:2017mis, Gu:2018gmy}.
...
\paragraph{SO(10)} The flavor symmetry acting on $N_{\bf s} \mathbf{S} + N_{\bf c} \mathbf{C} + N_{\bf v} \mathbf{V}$ hypermultiplets is $U(N_{\bf s})_s \times U(N_{\bf c})_c \times Sp(N_{\bf v})_v$, reflecting  that the $SO(10)$ (conjugate) spinor representation is complex. For $N_{\bf s} + N_{\bf c} = 2$ and $N_{\bf v} = 2$, the character expansion of $\hat{Z}_1$ is given by 
\begin{align}
  \label{eq:so10-2s2v}
