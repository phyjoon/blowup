...
In $SU(N)_\kappa + N_f\mathbf{F} + 1\mathbf{AS}$ theory with $N_f + 2|\kappa| \leq N+4$, the ADHM quantum mechanics is the worldvolume theory of D1-branes, probing the D5-NS5-D7-O7${}^{-}$ brane configuration that realizes the gauge theory. The Witten index for the one and two D1-branes is given by
\begin{align}
  Z_1^\text{ADHM} &=\\
  Z_2^\text{ADHM} &=
\end{align}
We checked that the general 1-instanton expression \eqref{eq:1inst-formula} agrees with $Z_1^\text{ADHM}$ at $N=3,4,5$. $Z_2 = Z_2^\text{ADHM}$  was confirmed  as well, for $N=3,4,5$, where $Z_2$ is the solution of the recursion formulae \eqref{eq:recursion} with \eqref{eq:sun-nf-na}.
We remark that $Z_k = Z_k^\text{ADHM}$ captures an extra factor $Z_\text{extra} \neq 1$ when $N_f + 2|\k| = N+4$, displaying the BPS spectrum of escaping D1-branes from the D5-branes on which the gauge symmetry is supported. The appearance of the non-trivial $Z_\text{extra}$ is therefore an artifact of the string theory embedding, spurious from the 5d QFT perspective. It indicates our recursion formula implicitly assumes the D5-NS5-D7-O7${}^{-}$ brane system.\footnote{An exceptional case is the $SU(2)$ gauge theory, in which the antisymmetric hypermultiplet decouples and never affects the recursion formula. The corresponding $Z_k$ is the same as the Young tableau formula \eqref{eq:SUn-young}.}

\paragraph{Sp(N)}
The instanton partition function for $Sp(N)_\theta + N_f \mathbf{F}$ theory with $N_f \leq 2N+4$ can be obtained from the ADHM quantum mechanics of the D1-D5-O5${}^-$ brane system, engineering the gauge theory. For one and two D1-branes, the corresponding Witten index is given by
\begin{align}
  Z_1^\text{ADHM} &=\\
  Z_2^\text{ADHM} &=
\end{align}
Our 1-instanton formula \eqref{eq:1inst-formula} was found to be in agreement with $Z_1^\text{ADHM}$ at $N=2,3$. 


---


$Sp(N) + N_f \mathbf{F} + 1\mathbf{AS}$
