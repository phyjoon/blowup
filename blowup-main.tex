...
It turns out piece of the \eqref{eq:1loopvec}, \eqref{eq:1loophyp} that are not inside the PE eventually cancel out (due to $\vec{k} \to -\vec{k}$ and $\vec{a} \to -\vec{a}$ symmetry. Therefore, we simply evaluate the ratio inside the PE. This yields
\begin{align}
 ...
 g(\vec{k})^{H} &= \prod_{\vec{w} \in R} \textrm{PE} \left[ \frac{- e^{\vec{a} \cdot \vec{w}+ m^{\textrm{phy}}}}{(1-e^{\e_1})(1-e^{\e_2})} + \frac{e^{(\vec{a}+\vec{k}\e_1) \cdot \vec{w} + m^{\textrm{phy}}}}{(1-e^{\e_1})(1-e^{\e_2-\e_1})} + \frac{e^{(\vec{a}+\vec{k}\e_2) \cdot \vec{w} + m^{\textrm{phy}}}}{(1-e^{\e_1- \e_2})(1-e^{\e_2})} \right] , 
\end{align}
for the vector and hypermultiplets respectively. Both of them can be written as 
\begin{align}
 g(\vec{k}) =  \frac{\displaystyle \prod_{i} \prod_{\vec{w} \in R_i}  \textrm{PE} \left[ e^{\vec{a} \cdot \vec{w} - m^{\textrm{phy}}_i}A(\vec{k} \cdot \vec{w}, e^{\e_1} e^{\e_2})\right]}{\displaystyle \prod_{\vec{\a} \in \Lambda}  \textrm{PE} \left[ e^{\vec{a} \cdot \vec{\a}}A(\vec{k} \cdot \vec{\a}, e^{\e_1} e^{\e_2})\right] } \ , 
\end{align} 
...
& \quad \times \frac{\displaystyle \prod_{i} \prod_{\vec{w} \in R_i}  \CL_{\vec{k} \cdot \vec{w}} (\vec{a} \cdot \vec{w} - m^{\textrm{phy}}_i, \e_1, \e_2)}{\displaystyle \prod_{\vec{\a} \in \Lambda}  \CL_{\vec{k} \cdot \vec{\a}}(\vec{a} \cdot \vec{\a}, \e_1, \e_2)  }  \\
...
\end{split}
\end{align}
where $a_\g \equiv \vec{a}\cdot \vec{\g}$ and $a_\a \equiv \vec{a}\cdot \vec{\a}$. The one-instanton partition function can be written explicitly as 
\begin{align}
\begin{split}
...
where 
\begin{align}
 M(\g) \equiv \prod_{\vec{w} \in R} \CL_{\g \cdot w} (a_w, \e_1, \e_2) \ , 
\end{align}
with $a_w \equiv \vec{a} \cdot \vec{w}$. 
...
\end{align}
which we conjecture to be true for all the hypermultiplets with the representation with $|\g \cdot w| \le 1$. 
