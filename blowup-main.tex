...
\end{align}
% \footnote{The orbifold partition function can be also obtained in two different ways modulo some subtle scheme dependence related to wall-crossing. \cite{Ito:2013kpa}} 

%
% This simple picture does not necessarily hold when there are too many hypermultiplets. However, for many interesting cases with matter hypermultiplets \cite{Gottsche:2010ig}, the above relation turns out to be true.
...
\begin{align}
  % \CZ_\text{hyper}(\epsilon_1, \epsilon_2, m) = 
...
\begin{align} \label{eq:muC}
  \mu(C) = \int \left\{ \omega \wedge \text{Tr}\Big(\Phi F + \half \psi \wedge \psi\Big) + H\, \text{Tr}\Big( F \wedge F \Big) \right\}  \ .
\end{align} 
...
As we shrink the two-cycle $C$ to recover the flat $\IC^2$, the effect of  inserting $\mu(C)^d$ turns out to give a vanishing contribution due to the conservation law. To see this, we recall that the classical $U(1)_R$ symmetry is violated by $4h^\vee -2 \sum_i I_2(\mathbf{R}_i)$ 

instanton breaks the $U(1)_R$ symmetry 



to the a discrete subgroup $\IZ_{2h^\vee - \sum_i I_2(\mathbf{R}_i)}$, under which the operator \eqref{eq:muC} carries a charge of $+2$. Unless the discrete charges add up to zero, the correlation function vanishes. Therefore, expanding in powers of $t$, 
\begin{align}
...
Turning to the 5d $\CN=1$ gauge theory on $S^1$, $\mathcal{Q}^2$ now involves translation along the $S^1$. The Casimir operator $\Tr (\Phi^2)$ and its descendants are no longer well-defined operators, but instead we need to 
