...
%The right-hand side of the recursion relation \eqref{eq:recursion} only involves the instanton parts with $\ell , m < n$ when $\vec{k} \neq 0$. For $\vec{k}=0$, it involves the instanton parts with $\ell + m \le n$, but it never goes beyond $n$. Notice that we have a set of equations labeled by the parameter $d$. Whenever the recursion relation is satisfied for more than 3 values of $d$, we can solve 
...
\begin{table}[t!]
  ...
  \begin{tabular}{|c|c|c|c|c|}
    ...
    \hline \hline
    $SU(N)_\kappa$ & $N_f \boldsymbol{F}$ & Always  &    \eqref{eq:sun-nf} & \eqref{eq:sun-nf-range} \\
    \cline{1-5}
    \multirow{2}{*}{$SU(N)_\kappa$} & \multirow{2}{*}{$N_f \boldsymbol{F}+1\textbf{AS}$} & $N_f \le N-1$ &    \multirow{2}{*}{\eqref{eq:sun-nf-na}} & \multirow{2}{*}{\eqref{eq:bound}} \\\cline{3-3}
    ...
    \cline{1-3}
    $Sp(N)_{\theta=\pi}$ & $N_f\boldsymbol{F}+N_a\boldsymbol{A}$ & $N_a(N-1)+\lceil N_f/2\rceil\le  N$ & & \\
    \cline{1-3}
    $SO(2N)$ & $N_v\boldsymbol{V}+N_s\boldsymbol{S}+N_c\boldsymbol{C}$ & $N_v+2^{N-4}(N_s+N_c) \le  2N-4$ &  & \\
    \cline{1-3}
    $SO(2N+1)$ & $N_v\boldsymbol{V}+N_s\boldsymbol{S}$ & $N_v+2^{N-3}N_s \le 2 N-3$ & &   \\
    \cline{1-3}
    $E_6$ & $N_f\boldsymbol{F}+N_{\bar{f}}\boldsymbol{\bar{F}}$ & $N_f+N_{\bar{f}} \le 3 $ & &   \\
    \cline{1-3}
    $E_7$ & $N_f\boldsymbol{F}$ & $N_f \le 2$ & &   \\
    \cline{1-3}
    $E_8$ & $\emptyset$ &  & &   \\
    \hline
    \end{tabular}
    \caption{List of theories whose partition function is determined via blowup equations. The number of hypermultiplets are bounded so that there are at least 3 blowup equations. Notice that one can always find at least 3 blowup equations for $SU(N)_\kappa+N_{f}\boldsymbol{F}$ that \eqref{eq:SUn-young} satisfy. However, they behave badly at UV when $N_f+2|\kappa|>2N$.  }
    \label{tbl:list}
\end{table}
...
%    \hline
%    $SU(N)_\kappa$ & $N_f\times\boldsymbol{N}$ & $d-N/2-\kappa/2$ & $0 \le  d \leq N-|\kappa|-2N_f-1$\textcolor{red}{(?)}\\
%    \hline
