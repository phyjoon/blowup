...
There are multiple and distinct blow-up equations \eqref{eq:blowup5d} at different values of $d$, ranged over $0 \leq d \leq d_{\rm max}$ where $d_{\rm max} \equiv h^\vee +  \kappa_{\rm eff} - \frac{1}{2}\sum_{i} I(\mathbf{R}_i)$. Since at least 3 distinct equations are necessary to solve for the instanton partition function, the bootstrap method is only applicable for theories which satisfy $d_{\rm max} \geq 2$. The full list of corresponding theories are:
